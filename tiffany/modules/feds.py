...
from tiffany import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, SARDEGNA_USERS, GBAN_LOGS, LOGGER
from tiffany.modules.helper_funcs.chat_status import is_user_admin
from tiffany.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from tiffany.modules.helper_funcs.string_handling import markdown_parser
from tiffany.modules.disable import DisableAbleCommandHandler

import tiffany.modules.sql.feds_sql as sql
from tiffany.modules.helper_funcs.alternate import send_message
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "tiffany_fbanned_users.json"
                message.reply_document(
                    document=output,
                    filename="tiffany_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "tiffany_fbanned_users.csv"
                message.reply_document(
                    document=output,
                    filename="tiffany_fbanned_users.csv",
